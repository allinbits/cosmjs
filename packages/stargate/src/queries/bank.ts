...
export function setupBankExtension(base: QueryClient): BankExtension {
  ...
  const queryService = cosmos.bank.v1beta1.Query.create((method: any, requestData, callback) => {
  ...
  return {
    bank: {
      balance: async (address: string, denom: string) => {
        ...
        return responseData.length ? toObject(cosmos.base.v1beta1.Coin.decode(responseData)) : null;
      },
      unverified: {
        balance: async (address: string, denom: string) => {
