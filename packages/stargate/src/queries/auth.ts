...
export function setupAuthExtension(base: QueryClient): AuthExtension {
  ...
  const queryService = cosmos.auth.v1beta1.Query.create((method: any, requestData, callback) => {
      ...
      account: async (address: string) => {
        ...
        switch (account.type_url) {
          case "/cosmos.auth.v1beta1.BaseAccount": {
            return toObject(cosmos.auth.v1beta1.BaseAccount.decode(account.value));
          }
          default:
